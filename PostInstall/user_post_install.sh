...
function InstallAdditionalPackages() {
	...
	echo "${password}" | sudo -S pacman -S timeshift dosfstools ntfs-3g iw wpa_supplicant dialog intel-ucode lshw unzip htop wget openssh git gdb valgrind man tldr stow bash-completion reflector mpv fastfetch transmission-gtk meld tmux cmake clang lib32-clang bear ninja p7zip --noconfirm
}
